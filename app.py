...
if plot_df.empty:
    ...
    # Color-coding by altitude (meters)
    def _color_for_alt(a):
        try:
            if np.isnan(a):
                return [120, 120, 120]
        except Exception:
        ...
        if a < 16000:  # 14–16 km
            return [255, 87, 51]
        return [199, 0, 57]  # > 16 km

    plot_df["color"] = plot_df["alt"].apply(_color_for_alt)

    # Legend (HTML)
    _legend_html = """
    <div style='position:relative; margin: 8px 0 12px 0;'>
      <div style='display:flex; gap:16px; align-items:center; flex-wrap:wrap; font-size:13px;'>
        <span><span style='display:inline-block;width:12px;height:12px;border-radius:50%;background:#ffe633;border:1px solid rgba(0,0,0,0.2);margin-right:6px;'></span>< 12 km</span>
        <span><span style='display:inline-block;width:12px;height:12px;border-radius:50%;background:#ffc300;border:1px solid rgba(0,0,0,0.2);margin-right:6px;'></span>12–14 km</span>
        <span><span style='display:inline-block;width:12px;height:12px;border-radius:50%;background:#ff5733;border:1px solid rgba(0,0,0,0.2);margin-right:6px;'></span>14–16 km</span>
        <span><span style='display:inline-block;width:12px;height:12px;border-radius:50%;background:#c70039;border:1px solid rgba(0,0,0,0.2);margin-right:6px;'></span>> 16 km</span>
      </div>
      <div style='font-size:11px;color:#666;margin-top:6px;'>Colors by altitude (meters). Thresholds: 12, 14, 16 km.</div>
    </div>
    """
